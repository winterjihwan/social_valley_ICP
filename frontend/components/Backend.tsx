...
const Backend = () => {
  ...
  const createProfile = async (principalText: String) => {
    ...
    await backend.create_profile(principalText)
    console.log("Profile created")
  }

  const createIndex = async (principalText: String) => {
    console.log("Creating index")
    await backend.create_index(principalText)
    console.log("Index created")
  }

  const queryProfile = async (principalText: String) => {
    console.log("Querying profile")
    const profile = await backend.query_profile(principalText)
    const comments = await backend.query_comments(principalText)
  ...
  const queryIndex = async (principalText: String) => {
    ...
    const index = await backend.query_index(principalText)
  ...
  const rate = async (
    ...
    console.log("Rating")
    await backend.rate(comment, commentor, like, principal)
    console.log("Rated")
  }
  ...
  return (
    <div>
      ...
      <div>
        ...
        <button
          onClick={() => rate(rateComment, "commentor", true, ratePrincipal)}
        >
          Rate
        </button>
        <input
